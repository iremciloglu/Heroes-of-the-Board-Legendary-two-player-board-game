        ...
        if (turn % 2 == 0) {//Player ones turn
            ...
            for (int i = 0; i < p1_board.getRowNum(); i++) {// trying to implement hit
                ...
                for (int j = p1_board.getColumnNum()-1; j >= 0; j--) {
                    int tempHit = 3;
                    //int tempHit = p1_board.getPieces()[i][j].getDamagePerHit();//override yap
                   ...
                   // only one piece in the a row GETS HIt by the enemy!
                        if (p2_board.getPieces()[i][j].getCharacter() != ' ' || p2_board.getPieces()[i][j].getLife() != 0) {
                            cout << "Check: r:  " << i << "   c:" << j << endl;
                            if (tempHit <= p2_board.getPieces()[i][j].getLife()) {
                                int newLife = p2_board.getPieces()[i][j].getLife() - tempHit;
                                p2_board.getPieces()[i][j].setLife(newLife);
                                int newl = p2_board.getPieces()[i][j].getLife();
                                cout << "new life" << newl;
                                tempHit = 0;
                            }
                            else {
                                tempHit = tempHit - p2_board.getPieces()[i][j].getLife();
                                p2_board.getPieces()[i][j + 1].setCharacter(' '); //This means its dead
                                p2_board.getPieces()[i][j + 1].setLife(0);
                                p2_board.getPieces()[i][j].setCharacter(' '); //This means its dead
                                p2_board.getPieces()[i][j].setLife(0);
                            }
                        }
                        else if (i == 0) {
        ...
        if (turn % 2 == 1) {//Player twos turn
            ...
            for (int i = 0; i < p2_board.getRowNum(); i++) {// trying to implement hit
                ...
                for (int j = p2_board.getColumnNum() - 1; j >= 0; j--) {
                    ...
                    //int tempHit = 3;
                    cout << "Hit:" << tempHit << endl;
                    // it enters the loop 3 times with the same coordinate i guess, idk why!!!!
                    cout << "j = " << j << endl;
                    if (p1_board.getPieces()[i][j].getCharacter() != ' ' || p1_board.getPieces()[i][j].getLife() != 0) {
                        cout << "Check: r:  " << i << "   c:" << j << endl;
                        cout << "Check: char:  " << p1_board.getPieces()[i][j].getCharacter() << "   life:" << p1_board.getPieces()[i][j].getLife() << endl;
                        if (tempHit <= p1_board.getPieces()[i][j].getLife()) {
                            int newLife = p1_board.getPieces()[i][j].getLife() - tempHit;
                            p1_board.getPieces()[i][j].setLife(newLife);
                            int newl = p1_board.getPieces()[i][j].getLife();
                            cout << "new life" << newl;
                            tempHit = 0;
                        }
                        else {
                            tempHit = tempHit - p1_board.getPieces()[i][j].getLife();
                            p1_board.getPieces()[i][j + 1].setCharacter(' '); //This means its dead
                            p1_board.getPieces()[i][j + 1].setLife(0);
                            p1_board.getPieces()[i][j].setCharacter(' '); //This means its dead
                            p1_board.getPieces()[i][j].setLife(0);
                        }
                    }
                    else if (i == 0) {
